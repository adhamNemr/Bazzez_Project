...
exports.createOrder = async (req, res) => {
  try {
    ...
    const productTotal = orderDetails.reduce((total, product) => {
      ...
      if (Array.isArray(product.comments)) {
        product.comments.forEach((c) => {
          ...
          if (p < 0)
            manualDiscountTotal +=
              Math.abs(p) * (Number(product.quantity) || 0);
          else addonsPrice += p;
        });
      }
    ...
    const finalTotal = Math.max(
      productTotal - discountValue - manualDiscountTotal + finalDeliveryPrice,
      0,
    );

    // 🚀 TRANSACTIONAL ORDER CREATION & INVENTORY DEDUCTION
    const orderResult = await sequelize.transaction(async (t) => {
      ...
      const nextSerial = lastOrder
        ? (Number(lastOrder.dailySerial) || 0) + 1
        : 1;
      ...
      // ✅ ATOMIC INVENTORY DEDUCTION (Inside Transaction)
      const recipeMap = {};
      ...
      for (const item of orderDetails) {
        if (item.productId && item.quantity > 0) {
          ...
          // 2. Deduct from Recipes/Ingredients
          ...
          // 3. Deduct from Direct Inventory Item
          await Inventory.decrement("quantity", {
            by: item.quantity,
          ...
          if (item.variant) {
            ...
            if (invItem && invItem.variants) {
              let vs =
                typeof invItem.variants === "string"
                  ? JSON.parse(invItem.variants)
                  : invItem.variants;
              vs = vs.map((v) => {
                ...
                if (item.color !== undefined) {
                  ...
                  if (vColor === itemColor && (vSize === itemSize || !vSize)) {
                    return {
                      ...v,
                      quantity: (v.quantity || 0) - item.quantity,
                    };
                  }
                  return v;
                }
                ...
                if (
                  ...
                  (vLabel &&
                    item.variant &&
                    item.variant.startsWith(vLabel + " "))
                ) {
                  return { ...v, quantity: (v.quantity || 0) - item.quantity };
                }
                return v;
              });
              await invItem.update({ variants: vs }, { transaction: t });
            }
          }
        }
      }
    ...
    printReceipt(orderData).catch((e) =>
      console.error("🖨️ Printing Error (Background):", e),
    );

    // ✅ Enqueue for Sync
    syncService
      .enqueue("INSERT", "orders", order.id, order.toJSON())
    ...
    res
      .status(500)
      .json({ message: "❌ فشل إنشاء الطلب", error: error.message });
  }
};
