...
const getAllInventory = async (req, res) => {
  try {
    // 🛡️ Self-healing: Ensure all products are in inventory
    const products = await Product.findAll({ raw: true });
    const inventoryNames = (
      await Inventory.findAll({ attributes: ["name"], raw: true })
    ).map((i) => i.name);

    for (const p of products) {
      if (!inventoryNames.includes(p.name)) {
        console.log(`⚠️ Auto-adding missing product to inventory: ${p.name}`);
        await Inventory.findOrCreate({
          where: { name: p.name },
          defaults: {
            quantity: 0,
            cost: p.wholesalePrice || p.price,
            min: 5,
            total: 0,
            variants: [],
          },
...
const addInventoryItem = async (req, res) => {
  try {
    ...
    // ✅ Enqueue for Sync
    syncService
      .enqueue("INSERT", "inventory", newItem.id, newItem.toJSON())
...
const updateInventoryItem = async (req, res) => {
  try {
    ...
    // ✅ Enqueue for Sync
    syncService
      .enqueue("UPDATE", "inventory", id, item.toJSON())
      .catch((err) => console.error("Sync queue error:", err));
...
const deleteInventoryItem = async (req, res) => {
  try {
    ...
    // ✅ Enqueue for Sync
    syncService
      .enqueue("DELETE", "inventory", id, { id })
      .catch((err) => console.error("Sync queue error:", err));
