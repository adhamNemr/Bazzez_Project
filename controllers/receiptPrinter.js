...
async function getStoreSettings() {
  try {
    ...
    console.error(
      "❌ Failed to fetch settings for printer, using defaults",
      err,
    );
    return branding;
  }
...
async function printReceipt(orderData) {
  ...
  try {
    ...
    try {
      ...
      console.log(
        "⚠️ Not running in Electron environment, skipping silent print.",
      );
    }

    const subtotal =
      orderData.orderDetails?.reduce((total, item) => {
        const basePrice = parseFloat(item.price) || 0;
        const quantity = parseFloat(item.quantity) || 0;

        let addonsTotal = 0;
        if (Array.isArray(item.comments)) {
          addonsTotal = item.comments.reduce(
            (sum, c) =>
              sum + (parseFloat(c.price) > 0 ? parseFloat(c.price) : 0),
            0,
          );
        }
    ...
    // 2. Thermal Print
    await printThermal(orderData, calculatedTotal, storeConfig);
  } catch (error) {
...
function generatePDF(
  ...
  try {
    ...
    doc
      .fontSize(14)
      .text(fixArabic(storeConfig.restaurantName), { align: "center" });
    doc.moveDown(0.2);
    doc
      .fontSize(32)
      .text(`#${orderData.dailySerial || orderData.id}`, { align: "center" });
    doc
      .fontSize(8)
      .text("V O R T E X  P O S", { align: "center", opacity: 0.3 });
    doc.moveDown(0.4);
    ...
    doc.fontSize(10);
    const infoY = doc.y;
    doc.text(fixArabic(`التاريخ: ${orderData.orderDate}`), leftX, infoY, {
      align: "right",
      width: pageWidth - 10,
    });
    doc.moveDown(0.3);
    ...
    const cleanName = (val) =>
      val?.includes("تيك أوي") || val?.includes("--") || !val ? "--" : val;
    doc.text(
      fixArabic(`العميل: ${cleanName(orderData.customerName)}`),
      leftX,
      doc.y,
      { align: "right", width: pageWidth - 10 },
    );

    if (orderData.customerPhone && orderData.customerPhone !== "0000000000") {
      doc.moveDown(0.3);
      doc.text(fixArabic(`الهاتف: ${orderData.customerPhone}`), leftX, doc.y, {
        align: "right",
        width: pageWidth - 10,
      });
    }
    ...
    orderData.orderDetails?.forEach((item) => {
      ...
      doc.text(`x${item.quantity}`, colQtyX, curY, {
        width: 35,
        align: "center",
      });
      doc.text(fixArabic(item.name), colItemX, curY, {
        width: 101,
        align: "right",
      });
      ...
      if (item.comments && item.comments.length > 0) {
        item.comments.forEach((c) => {
          ...
          doc.fontSize(9).text(fixArabic(`└─ ${txt}`), colItemX, doc.y, {
            width: 101,
            align: "right",
            color: "#555555",
          });
    ...
    const drawTotalRow = (label, value) => {
      doc.text(fixArabic(label), centerX, doc.y, {
        width: centerX - 5,
        align: "right",
      });
    ...
    if (deliveryPrice > 0)
      drawTotalRow("خدمة التوصيل:", deliveryPrice.toFixed(2));
    if (discount > 0) drawTotalRow("الخصم:", `-${discount.toFixed(2)}`);

    doc.moveDown(0.4);
    doc
      .lineWidth(1.5)
      .moveTo(leftX + 20, doc.y)
      .lineTo(rightX - 20, doc.y)
      .stroke();
    doc.moveDown(0.6);
    ...
    doc
      .fontSize(24)
      .text(`${calculatedTotal.toFixed(2)} ${storeConfig.currency}`, {
        align: "center",
        underline: true,
      });
    ...
    doc
      .fontSize(9)
      .text(fixArabic(storeConfig.footerMessage), { align: "center" });
    doc.moveDown(0.5);
    doc.fontSize(7).text(`Vortex POS - ${new Date().toLocaleString("ar-EG")}`, {
      align: "center",
      opacity: 0.2,
    });
    ...
    doc.end();
    console.log(
      `✅ Professional Receipt #${orderData.id} Generated with Arabic Fonts.`,
    );
  } catch (error) {
 ...
 */
async function printThermal(orderData, calculatedTotal, storeConfig) {
    ...
    device.open(async function (error) {
      ...
      try {
        printer
          ...
          .size(1, 1)
          .text(`#${orderData.dailySerial || orderData.id}`)
        ...
        const cleanValue = (val, forbiddenKeywords = []) => {
          ...
          if (["0000000000", "0", "--", "Store", "Local"].includes(trimmed))
            return "-";
          return trimmed;
        };

        const cName = cleanValue(orderData.customerName, [
          "تيك أوي",
          "نقدي",
          "Guest",
        ]);
        ...
        orderData.orderDetails?.forEach((item) => {
          ...
          const finalPrice = (
            (parseFloat(item.price) + addonsTotal) *
            item.quantity
          ).toFixed(2);
          printer.text(
            `${item.quantity} x ${fixArabic(item.name)} : ${finalPrice}`,
          );
        });
        ...
        printer
          ...
          .text(
            fixArabic(
              `Total: ${calculatedTotal.toFixed(2)} ${storeConfig.currency}`,
            ),
          )
          .align("ct")
          .text("--------------------------------")
