...
if (!gotTheLock) {
  ...
  function logToUI(msg, type = "log") {
    ...
    mainWindow.webContents
      .executeJavaScript(`console.${type}("🖥️ SERVER: " + ${safeMsg})`)
      .catch(() => {});
  }
  ...
  function createWindow() {
    ...
    // 🔍 Auto-Scale Fix for Windows to match Mac proportions
    mainWindow.webContents.on("did-finish-load", () => {
      if (process.platform === "win32") {
        // Windows scaling is often too aggressive, zoom out slightly to match Mac logical feel
  ...
  app.whenReady().then(() => {
    ...
    ipcMain.on("print-receipt", (event, orderData) => {
      if (global.logToUI) {
        ...
        global.logToUI(
          `📦 INCOMING ORDER DETAILS: ${JSON.stringify(orderData?.orderDetails)}`,
        );
      }
      ...
      const printerName = (process.env.PRINTER_INTERFACE || "").replace(
        "printer:",
        "",
      );
      ...
      printWindow.webContents.on("did-finish-load", () => {
        ...
        setTimeout(() => {
          ...
          printWindow.webContents.print(printOptions, (success, errorType) => {
            if (!success) {
              if (global.logToUI)
                global.logToUI(`❌ Print failed: ${errorType}`, "error");
            } else {
              if (global.logToUI)
                global.logToUI(`✅ Print success for Order #${orderData.id}`);
            }
            printWindow.close();
          });
        }, 300);
    ...
    ipcMain.on("get-receipt-data", (event) => {
      if (global.logToUI) {
        global.logToUI(
          `📡 Receipt page requested data. Sending Order #${global.lastOrderData?.id}`,
        );
        global.logToUI(
          `📤 OUTGOING ORDER DETAILS: ${JSON.stringify(global.lastOrderData?.orderDetails)}`,
        );
      }
