...
const systemMode = localStorage.getItem("systemMode") || "retail"; // Default to retail

const t = {
  ar: {
    pageTitle:
      systemMode === "restaurant"
        ? "نافذة الطلبات - Vortex POS"
        : "نافذة البيع - Vortex POS",
    headerPill:
      systemMode === "restaurant" ? "منصة طلبات المطعم" : "منصة البيع المباشر",
    home: "الرئيسية",
    operator: "المشغل",
    searchCustomer:
      systemMode === "restaurant"
        ? "ابحث عن طاولة أو رقم طلب..."
    ...
    instapay: "إنستاباي",
    vcash: "فودافون كاش",
    cartSummary: systemMode === "restaurant" ? "تفاصيل الوجبات" : "ملخص السلة",
    emptyCart:
      systemMode === "restaurant"
        ? "لم يتم اختيار وجبات بعد"
        : "السلة فارغة حالياً",
    subtotal: "المجموع الفرعي",
    delivery: systemMode === "restaurant" ? "خدمة التوصيل" : "توصيل",
    total: "الإجمالي",
    discount: "الخصم",
    checkout: systemMode === "restaurant" ? "تأكيد الطلب" : "إتمام الطلب",
    selectDelivery: "اختر منطقة التوصيل",
    items: systemMode === "restaurant" ? "وجبات" : "أصناف",
  ...
  en: {
    pageTitle:
      systemMode === "restaurant"
        ? "Order Hub - Vortex POS"
        : "Cashier Workspace - Vortex POS",
    headerPill:
      systemMode === "restaurant"
    ...
    instapay: "InstaPay",
    vcash: "V-Cash",
    cartSummary: systemMode === "restaurant" ? "Meal Details" : "Cart Summary",
    emptyCart:
      systemMode === "restaurant"
    ...
    total: "Total",
    discount: "Discount",
    checkout: systemMode === "restaurant" ? "Confirm Order" : "Complete",
    selectDelivery: "Select Delivery Area",
    items: systemMode === "restaurant" ? "meals" : "items",
...
function initUser() {
  ...
  let storedUsername =
    localStorage.getItem("username") || t[currentLang].operator;
  if (activeUserBadge) {
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 401) {
        Swal.fire({ icon: "error", title: t[currentLang].sessionExpired }).then(
...
function showCategory(category) {
  ...
  products.forEach((product) => {
    ...
    const isSuperSimple =
      variants.length === 1 && sizes.length === 1 && sizes[0] === "مقاس واحد";
    ...
    card.onclick = () => {
      if (isSuperSimple) {
        addVariantToOrder(
          ...
          product.name,
          variants[0].price,
          variants[0].name || variants[0].color,
          sizes[0],
        );
      } else if (hasVariants) {
...
function getSizesForCategory(category, productName) {
  ...
  const isHalfSleeve =
    productName.includes("نص كم") || productName.includes("نص كم");
  const isAlAzzLongSleeve =
    productName.includes("العز") && productName.includes("فخامه كم طويل");
...
function showVariantModal(product) {
  ...
  const initialButtonStyle =
    hasFabrics && !isSingleFabric ? "opacity: 0.4; pointer-events: none;" : "";
            ...
            <div style="margin-bottom: 1rem; text-align: center;">
                ...
                ">
                    ...
                    `,
                            ...
                            .join("")
                        : `<p style="opacity:0.5;">${isAr ? "لا يوجد خامات معرفة" : "No fabrics defined"}</p>`
                    }
                </div>
            </div>
...
function selectFabricForOrder(fabric, price) {
  ...
  const sizes = getSizesForCategory(
    currentProductForModal.category,
  ...
  const isAlAzzLongSleeve =
    currentProductForModal.name.includes("العز") &&
    fabric.includes("فخامه كم طويل");
  ...
  if (isHalfSleeve || isAlAzzLongSleeve) {
    ...
    const originalSizes = getSizesForCategory(
      currentProductForModal.category,
      currentProductForModal.name,
    );
...
function renderSizeButtons(sizes) {
  ...
  const initialButtonStyle = selectedFabricForModal
    ? ""
  ...
  const isNumericCategory =
    currentProductForModal.category === "جلباب أطفالي" ||
    currentProductForModal.category === "سراويل" ||
    (currentProductForModal.category === "جلباب رجالي" &&
      !sizes[0].includes("S"));
...
function handleSizeClick(size) {
  ...
  if (hasFabrics && !selectedFabricForModal) {
    Swal.fire({
      ...
      text: isAr
        ? "يرجى اختيار الخامة / اللون أولاً"
        : "Please select fabric / color first",
...
function addVariantToOrder(id, name, price, color, size) {
  ...
  const isFaqiSpecial =
    name.startsWith("الفقي") &&
    (size.startsWith("64") || size.includes("2XL") || size.includes("3XL"));

  // Rule 2: 'سراويل' category with size "32" or "34"
  const isSaruwalSpecial =
...
function renderOrderSummary() {
  ...
  discountEl.textContent =
    totalDiscount === 0 ? "0.00" : totalDiscount.toFixed(2);
...
function saveCustomComment() {
  ...
  const price =
    parseFloat(document.getElementById("manualPriceInput").value) || 0;
  const discount =
    parseFloat(document.getElementById("manualDiscountInput").value) || 0;
...
function setPaymentMethod(method) {
  ...
  document
    .querySelector(`.payment-btn[data-method="${method}"]`)
