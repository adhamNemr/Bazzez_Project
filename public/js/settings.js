...
const t = {
  ar: {
    ...
    resetWarnText:
      "سيتم مسح جميع الفواتير والمبيعات والمصروفات والعمليات بالكامل محلياً وعلى السحابة! هذا الإجراء لا يمكن التراجع عنه!",
  ...
  en: {
    ...
    resetWarnText:
      "All invoices, sales, expenses, and transaction logs will be permanently wiped locally and from the cloud! This action CANNOT be undone!",
...
document.addEventListener("DOMContentLoaded", async () => {
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            throw new Error(
              isAr
                ? "فشل في الاتصال بالسيرفر لتصفير البيانات"
                : "Server failed to reset database",
            );
          }
        } catch (error) {
          Swal.fire({
  ...
  // 📡 Force Sync All Data Button
  const forceSyncBtn = document.getElementById("force-sync-btn");
  if (forceSyncBtn) {
    forceSyncBtn.addEventListener("click", async () => {
      const result = await Swal.fire({
        title: isAr
          ? "هل تريد رفع جميع البيانات إلى السحابة؟"
          : "Do you want to push all data to the cloud?",
        text: isAr
          ? "سيتم رفع جميع المنتجات، المخزون، الفواتير، والمصروفات إلى السحابة (آمن ولا يحذف أي بيانات)!"
          : "All products, inventory, orders, and expenses will be pushed to the cloud (safe, no data deletion)!",
        icon: "info",
        showCancelButton: true,
        confirmButtonColor: "#059669",
        cancelButtonColor: "#64748b",
        confirmButtonText: isAr
          ? "نعم، ارفع جميع البيانات!"
          : "Yes, push everything!",
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire({
          title: isAr
            ? "جاري رفع البيانات إلى السحابة..."
            : "Pushing data to the cloud...",
          text: isAr
            ? "من فضلك لا تغلق الصفحة!"
        ...
        try {
          const response = await fetch("/api/sync/force-push-all", {
            method: "POST",
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
          ...
          if (response.ok) {
            Swal.fire({
              ...
              text: isAr
                ? "تم رفع جميع البيانات المحلية إلى السحابة!"
                : "All local data has been pushed to the cloud!",
              confirmButtonColor: "var(--primary)",
            });
          } else {
            throw new Error(
              isAr ? "فشل في رفع البيانات!" : "Failed to push data!",
            );
          }
        } catch (error) {
          Swal.fire({
