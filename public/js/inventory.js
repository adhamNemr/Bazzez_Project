...
const t = {
  ar: {
    ...
    msgDeleted: "🗑️ تم الحذف نهائياً.",
    msgError: "⚠️ حدث خطأ ما.",
    confirmDelete: "هل أنت متأكد من الحذف؟",
    noResults: "لم يتم العثور على نتائج مطابقة",
    tableId: "ID",
    tableName: systemMode === "restaurant" ? "اسم الخامة" : "اسم الصنف",
    tableQty: "الكمية",
  ...
  en: {
    ...
    msgDeleted: "🗑️ Deleted successfully.",
    msgError: "⚠️ Something went wrong.",
    confirmDelete: "Are you sure you want to delete?",
    noResults: "No matching results found",
    tableId: "ID",
    tableName: systemMode === "restaurant" ? "Ingredient Name" : "Item Name",
...
function renderInventory(items) {
  const tableBody = document.getElementById("product-table");
  const isRetail =
    (localStorage.getItem("systemMode") || "restaurant") === "retail";
  ...
  const tableRows = items
    .map((item) => {
      ...
      if (hasVariants) {
        const variantsSum = item.variants.reduce(
          (sum, v) => sum + parseFloat(v.quantity || 0),
          0,
        );
        totalQty = variantsSum;
        totalMin = item.variants.reduce(
          (sum, v) => sum + parseFloat(v.min || 0),
          0,
        );
        ...
        if (totalQty > variantsSum) {
          ...
          totalValue = item.variants.reduce(
            (sum, v) =>
              sum +
              parseFloat(v.quantity || 0) *
                parseFloat(v.cost || item.cost || 0),
            0,
          );
        }
      ...
      const formattedMin = hasVariants
        ? isRetail
          ...
          : totalMin.toFixed(2)
        : isRetail
          ? Math.round(item.min || 0)
          : parseFloat(item.min || 0).toFixed(2);

      const expiryWarningHTML = isNearExpiry
        ? `<i class="fas fa-exclamation-circle expiry-pulse" title="${isAr ? "قرب الانتهاء" : "Expiring Soon"}"></i>`
        : "";
      ...
      if (hasVariants) {
        ...
        const branchIcon = isAr
          ? '<i class="fas fa-level-down-alt fa-rotate-90" style="margin-left: 10px; color:#cbd5e1;"></i>'
          : '<i class="fas fa-level-up-alt fa-rotate-90" style="margin-right: 10px; color:#cbd5e1;"></i>';

        item.variants.forEach((variant, vIdx) => {
          const variantQty = isRetail
            ? Math.round(variant.quantity)
            : parseFloat(variant.quantity).toFixed(2);
...
function attachInventoryListeners() {
  ...
  tableBody.querySelectorAll(".parent-row").forEach((row) => {
    ...
    row.addEventListener("click", (e) => {
      ...
      if (!isExpanded) {
        tableBody
          .querySelectorAll(".parent-row.expanded-row")
          .forEach((other) => {
  ...
  tableBody
    .querySelectorAll("tr:not(.parent-row):not(.tree-child-row)")
    .forEach((row) => {
      if (row.dataset.id) {
  ...
  tableBody.querySelectorAll(".tree-child-row").forEach((row) => {
    ...
    row.onclick = async (e) => {
      ...
      if (action === "add-variant") {
        const newV = await openVariantEntryModal(isAr, t[currentLang], {
          cost: item.cost,
        });
        if (newV)
          handleEdit(item.id, {
            ...item,
            variants: [...(item.variants || []), newV],
          });
      } else if (vidx !== undefined) {
        const variant = item.variants[vidx];
        const updated = await openVariantEntryModal(
          isAr,
          t[currentLang],
          variant,
        );
    ...
    preConfirm: () => {
      ...
      if (!name)
        return Swal.showValidationMessage(
          isAr ? "يرجى إدخال الاسم" : "Name is required",
        );
      ...
      if (price <= 0)
        return Swal.showValidationMessage(
          isAr ? "يرجى إدخال سعر البيع" : "Selling price is required",
        );
      return {
        name,
        color: name,
  ...
  const { value: formValues, isConfirmed } = await Swal.fire({
            ...
            <div style="padding: 1rem; text-align: ${isAr ? "right" : "left"};">
                ...
                <div id="variants-summary-list" style="margin-top: 1.5rem; border-top: 2px dashed #e2e8f0; padding-top: 1rem;">
                    ...
                    `,
                            ...
                            .join("")
                        : `<p style="text-align:center; opacity:0.5;">${isAr ? "لا يوجد تفريعات" : "No variants"}</p>`
                    }
                </div>
    ...
    didOpen: () => {
      const getCur = () => ({
        ...
        expiryDate: !isRetail
          ? document.getElementById("swal-expiry").value
          : "",
      ...
      document.getElementById("btn-add-variant-modal").onclick = async () => {
        const cur = getCur();
        const v = await openVariantEntryModal(isAr, langT, {
          min: cur.min,
          cost: cur.cost,
        });
        if (v) {
...
async function selectItem(item, preExistingData = null) {
  ...
  const {
    ...
    preConfirm: () => {
      ...
      if (!name)
        return Swal.showValidationMessage(
          isAr ? "يرجى إدخال اسم الصنف" : "Name is required",
        );

      const parentQty =
        parseFloat(document.getElementById("swal-edit-qty").value) || 0;
      const variantsSum = state.variants.reduce(
        (sum, v) => sum + parseFloat(v.quantity || 0),
        0,
      );
...
async function handleFormSubmit(data) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      Swal.fire({
        icon: "error",
        title: t[currentLang].msgError,
        text: err.error || `Server: ${res.status}`,
      });
    }
  } catch (err) {
...
async function handleEdit(id, data) {
  if ((typeof id === "string" && id.includes("-")) || parseInt(id) >= 1000) {
    Swal.fire({
      icon: "info",
      title: isAr
        ? "هذه بيانات تجريبية (Mock Data) للتوضيح فقط ولا يمكن تعديلها في قاعدة البيانات حالياً"
        : "This is mock data for demonstration and cannot be edited in the DB yet.",
    });
    return;
  }
  ...
  const token = localStorage.getItem("token");
  Swal.fire({
    title: isAr ? "جاري التحديث..." : "Updating...",
    allowOutsideClick: false,
    didOpen: () => Swal.showLoading(),
  });
...
async function handleDelete(id) {
  if ((typeof id === "string" && id.includes("-")) || parseInt(id) >= 1000) {
    Swal.fire({
      icon: "info",
      title: isAr
        ? "هذه بيانات تجريبية (Mock Data) للتوضيح فقط ولا يمكن حذفها"
        : "This is mock data and cannot be deleted.",
    });
    return;
  }
  ...
  try {
    ...
    if (res.ok) {
      Swal.fire({
        icon: "success",
        title: t[currentLang].msgDeleted,
        timer: 1500,
        showConfirmButton: false,
      });
      fetchInventory(true);
    ...
    if (filter === "low-stock") {
      filtered = filtered.filter((item) => {
        ...
        if (hasVariants) {
          ...
          const totalMin = item.variants.reduce(
            (sum, v) => sum + parseFloat(v.min || 0),
            0,
          );
          const hasLowVariant = item.variants.some(
            (v) => parseFloat(v.quantity || 0) <= parseFloat(v.min || 0),
          );
...
function exportToExcel() {
  try {
    if (!allInventory || allInventory.length === 0) {
      Swal.fire({
        icon: "warning",
        title: isAr ? "لا توجد بيانات لتصديرها" : "No data to export",
      });
      return;
    }
    ...
    allInventory.forEach((item) => {
      ...
      try {
        if (item.createdAt)
          addedDate = new Date(item.createdAt).toISOString().split("T")[0];
        if (item.expiryDate)
      ...
      if (hasVariants) {
        ...
        totalValue = variants.reduce(
          (sum, v) =>
            sum +
            parseFloat(v.quantity || 0) * parseFloat(v.cost || item.cost || 0),
          0,
        );
      } else {
        totalValue = totalQty * parseFloat(item.cost || 0);
      ...
      excelData.push({
        ...
        [isAr ? "سعر التكلفة (للوحدة)" : "Unit Cost"]: hasVariants
          ? "---"
          : parseFloat(item.cost || 0).toFixed(2),
        [isAr ? "سعر البيع" : "Selling Price"]: hasVariants
          ? "---"
          : parseFloat(item.price || item.cost || 0).toFixed(2),
        ...
        variants.forEach((v) => {
          ...
          try {
            if (v.updatedAt || v.createdAt) {
              vDate = new Date(v.updatedAt || v.createdAt)
                .toISOString()
                .split("T")[0];
            }
          } catch (e) {}
          ...
          excelData.push({
            [isAr ? "الكود" : "Code"]: "",
            [isAr ? "اسم الصنف / الخامة" : "Item / Fabric Name"]: isAr
              ? `  ↳ ${v.name || "بدون اسم"}`
              : `  ↳ ${v.name || "Unnamed"}`,
            [isAr ? "الكمية" : "Quantity"]: parseFloat(v.quantity || 0),
            [isAr ? "الحد الأدنى" : "Min Limit"]: parseFloat(v.min || 0),
            [isAr ? "سعر التكلفة (للوحدة)" : "Unit Cost"]: parseFloat(
              v.cost || item.cost || 0,
            ).toFixed(2),
            [isAr ? "سعر البيع" : "Selling Price"]: parseFloat(
              v.price || v.cost || item.cost || 0,
            ).toFixed(2),
            [isAr ? "إجمالي قيمة المخزون" : "Total Stock Value"]: (
              parseFloat(v.quantity || 0) * parseFloat(v.cost || item.cost || 0)
            ).toFixed(2),
    ...
    XLSX.writeFile(
      wb,
      `vortex_inventory_${new Date().toISOString().split("T")[0]}.xlsx`,
    );
